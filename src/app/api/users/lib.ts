...
export type OriginalUser = (typeof mockUsers)["users"][0];

export const groupUsersByDepartment = (users: (typeof mockUsers)["users"]) => {
  const result = new Map<string, User>();
  const tempAges = new Map<string, { min: number; max: number; range: string }>();
  ...
  users.forEach((user) => {
    ...
    const newData = result.get(dept);
    if (!newData) return;

    if (user.gender === "male") newData.male += 1;
    else if (user.gender === "female") newData.female += 1;

    if (user.hair.color)
      newData.hair[user.hair.color] = !newData.hair[user.hair.color]
        ? 1
        : newData.hair[user.hair.color] + 1;

    if (user.address.postalCode) {
      newData.addressUser[`${user.firstName}${user.lastName}`] = user.address.postalCode;
    }

    if (user.age) {
      if (tempAges.has(dept)) {
        const temp = tempAges.get(dept);
        if (!temp) return;
        if (user.age < temp.min) temp.min = user.age;
        if (user.age > temp.max) temp.max = user.age;
        temp.range = `${temp.min}-${temp.max}`;
        tempAges.set(dept, temp);
      } else {
        tempAges.set(dept, { min: user.age, max: user.age, range: `${user.age}-${user.age}` });
      }
      newData.ageRange = tempAges.get(dept)?.range || "";
    }

    result.set(dept, newData);
  });
